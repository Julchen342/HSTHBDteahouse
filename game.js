...
function drawObjects() {
  ...
  for (const obj of state.objects) {
    ctx.font = `${obj.kind === "bad" ? 35 : 37}px Apple Color Emoji, Segoe UI Emoji, sans-serif`;
  ...
  function drawPlayer() {
    ...
    ctx.beginPath();
    ctx.ellipse(player.x, player.y + 30, 44, 11, 0, 0, Math.PI * 2);
    ctx.fill();
...
ctx.shadowOffsetY = 0;
  ...
  function drawPauseScreen() {
    ...
    ctx.font = "700 18px system-ui, sans-serif";
    ctx.fillText("按空格或 ⏸️ 继续", W / 2, H / 2 + 40);
    ctx.restore();
  }
